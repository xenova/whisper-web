...
export default function AudioPlayer(props: { audioUrl: string }) {
    ...
    const createAudioPlayer = () => {
        ...
        _audioPlayer.ondurationchange = () =>
            !isNaN(_audioPlayer.duration) && setDuration(_audioPlayer.duration);
        _audioPlayer.ontimeupdate = () =>
            setCurrentTime(_audioPlayer.currentTime);
            ...
            <div className='relative z-10 p-4'>
                <div className='flex w-[41rem] rounded-lg bg-white shadow-xl shadow-black/5 ring-1 ring-slate-700/10'>
                    <div className='flex items-center space-x-4 py-4 px-6'>
                        <svg className='h-6 w-6 flex-none cursor-pointer' fill='none' onClick={
                            () => {
                        ...
                        <svg className='h-6 w-6 flex-none cursor-pointer' fill='none' onClick={
                            () => {
                                audioPlayer.currentTime = Math.min(audioPlayer.currentTime + 15, audioPlayer.duration - 1e-3);
                            }
                        }>
                            <path
